...
void Q5calculateRawData(char* ps, int(&userId)[ChatMSGS], int(&msgSize)[ChatMSGS], int(&totalWords)[ChatMSGS], int(&words)[ChatMSGS][Size])
{
	...
	for (int i = 0; i < ChatMSGS; i++)
	{
		...
		while (*(ps + (i * Size) + j) == 32) j++;

		//for (j = j; *(ps + (i * Size) + j) != '\0'; j++)
		while (*(ps + (i * Size) + j) != '\0')
		{
			if (*(ps + (i * Size) + j) != 32 &&
				((*(ps + (i * Size) + j + 1) == 32) ||
...
{
	char* ps{ &s[0][0] }, * psm{ &sm[0][0] };	// *(psm + (smi * UIWidth) + smj)
	...
	// see what data is coming
	//for (int i = 0; i < ChatMSGS; i++)
	//{
	//	cout << endl << userId[i] << " | " << msgSize[i] << " | " << totalWords[i] << " | ";
	...
	while (*ps != '\0')
	{
		if ((*ps >= 65 && *ps <= 90) || (*ps >= 97 && *ps <= 122))
		{
			...
			*ps = (*ps >= 97 && *ps <= 122) ? *ps : (*ps + 32);
			...
			while (*pa != '\0') { if (*pa == *ps && (*++pa += 1) && (notAvailable = false)) break; pa += 2; }

			if (notAvailable && (*pa = *ps)) *++pa = 1;
		}
		ps++;
	}
}
...
void Q1()
{
	...
	cout << endl << "Words" << " : " << "Frequency";
	for (int i = 0; ans[i][0] != '\0'; i++)
		cout << endl << setw(3) << ans[i][0] << "   : " << setw(3) << (int)ans[i][1];	// (int)ans[i][1] = storing hex & converted to int to go beyond 9 value
...
void Q3findLongestString(char(s)[Strs][Size], int(&lss)[Strs], int(&mx))
{
	...
	{
		for (int j = 0; *(ps + (i * Size) + j) != '\0'; j++)
			(*plss)++;

		if (*plss > *maxLen) *maxLen = *plss;
	}
...
void Q3()
{
	...
	cout << endl << "Longest Strings : " << endl << endl;
...
void Q4InputArray(int(&a)[PAGES][ROWS][COLS], bool inputManually = false)
{
	...
	cout << endl << "Page No. | Row No. | Col No. : " << endl << endl;
...
void Q4()
{
	...
	// if dont want to input manually default value(1 to 27)
	Q4InputArray(arr);
...
*/
//void Q5setModel(char(s)[Strs][Size], char(&sm)[Size][UIWidth])
//{
//	char* ps{ &s[0][0] }, * psm{ &sm[0][0] };
//	int temp{0}, width{0}, n{}, userId{};
//
//	for (int i = 0; i < Strs; i++)
//	{
//		userId = (i % 2 == 0) ? 1 : 2;
...
//					j--;
//				}
//				else if (width > 4 && width <= UIWidth)
//				{
//					*(psm + temp) = *(ps + (i * Size) + j);
//					temp++;
//					width++;
//				}
//				else if(width > UIWidth)
//				{
//					*(psm + temp++) = '\0';
//					width = 1;
//				}
...
void Q5viewChat(char(arr)[Size][UIWidth], int(r), bool mobileView = false)
{
	...
	if (mobileView)
	{
		...
		cout << "\t" << c[4] << " " << c[8] << c[8] << c[8] << c[8] << c[8] << "   " << c[4] << " " << 0 << " " << c[6] << c[6] << c[6] << c[6] << " " << c[5] << " " << c[4] << "  8:30AM " << c[4] << endl;
...
void Q5()
{
	...
	{ " UA1 1234567899876543211234 Helllo mister ramchandre, Nice to meet you how are you doing today?" },
	{ " UB1 hlloo miss ragighjkhjgkghjkhkgjhkhjkrning pleasure is all my im good " },
	{ "UA2 Helllo mister ramchandre, Nice to meet you how are you doing today? " },
	{ " UB2 hlloo miss ragighjkhjgkghjkhkgjhkrhjkning pleasure is all my im good" },
	{ " UA3 Helllo mister ramchandre, Nice to meet you how are you doing today? " },
	{ "UB3 hlloo miss ragighjkhjgkghjkhkgjhekhjkning pleasure is all my im good " }
	};
...
void main()
{
	...
	do {
		...
		// NOTE :
		// Q4) if dont want to input 27 times manually go to Q4() function and read comments...
